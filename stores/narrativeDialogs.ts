...
interface ClaimFeedbackDialogState {
  ...
  claimFeedbackScore: number | null; // 1 like, 0 dislike, null no feedback yet
  claimFeedbackComment: string;
  loading: boolean;
}
...
interface NarrativeFeedbackDialogState {
  isOpen: boolean;
  selectedNarrative: Narrative | null;
  narrativeFeedbackRating: number | null; // 1-5 stars, null no feedback yet
  narrativeFeedbackComment: string;
  loading: boolean;
}
...
export const useNarrativeDialogsStore = defineStore('narrativeDialogs', {
  ...
  actions: {
    ...
    async openClaimFeedbackDialog(claim: Claim, narrative: Narrative) {
      ...
      this.claimFeedbackDialog.claimFeedbackComment = feedbackResponse?.feedback_text ?? '';
    ...
    async sendClaimFeedback(score: number, comment: string) {
      ...
      return await apiService.sendClaimFeedback(
        this.claimFeedbackDialog.selectedClaim!.id,
        this.claimFeedbackDialog.selectedNarrative!.id,
        score,
        comment || null,
      ).catch((error) => {
    ...
    // Narrative Feedback Dialog Actions
    async openNarrativeFeedbackDialog(narrative: Narrative) {
      ...
      this.narrativeFeedbackDialog.narrativeFeedbackRating = feedbackResponse
        ? Math.round(feedbackResponse.feedback_score * MAX_NUMBER_OF_STARS)
        : null;
    ...
    async sendNarrativeFeedback(rating: number, comment: string) {
      const MAX_NUMBER_OF_STARS = 5;
      ...
      return await apiService.sendNarrativeFeedback(
        this.narrativeFeedbackDialog.selectedNarrative!.id,
        score,
        comment || null,
      ).catch((error) => {
